...
function getBaseUrl(siteUrl?: string) {
  // During template builds (or local dev) env vars may be unset; schemas should still render.
  return (process.env.NEXT_PUBLIC_SITE_URL || siteUrl || "http://localhost:3000").replace(
    /\/$/,
    ""
  );
}
