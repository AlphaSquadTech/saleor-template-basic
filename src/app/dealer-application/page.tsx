  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      dealerPageData?.fields
        .find((f) => f.toLowerCase() === "address line 2")
        ?.toLowerCase() !== "address line 2"
    ) {
      dealerPageData?.fields.forEach((field) => {
    ...
    try {
      ...
      const response = await fetch("https://smtp.wsm-dev.com/api/send-email", {
        ...
        body: JSON.stringify({
          ...
          subject:  dealerPageData?.emailSubject || "Dealer Application",
      ...
      <main className="h-full w-full">
        <div className="container mx-auto max-w-[1276px] py-24">
          <div className="text-center text-[var(--color-secondary-800)]">
            <p>Unable to load dealer application page. Please try again later.</p>
          </div>
        </div>
      </main>
    );
                  ...
                  return (
                    <div key={fieldKey} className="w-full">
                      <label className="block uppercase text-sm font-medium text-[var(--color-secondary-800)] leading-5 tracking-[-0.035px] mb-1.5">
                        {field}{" "}
                        {field.toLowerCase() === "address line 2" ? null : (
                      ...
                      <input
                        ...
                        required={
                          field.toLowerCase() === "address line 2"
                            ? false
                            : true
                        }
