...
export const useVehicleData = () => {
  ...
  const [rootTypesLoading, setRootTypesLoading] = useState(false);
  ...
  const initializeFromPairs = async (pairs: string) => {
    ...
    try {
      ...
      const selectedNamesWithTypes = levels
        .filter((level) => level.selectedValueId !== 0)
        .map((level) => `${level.typeName}: ${level.selectedValue}`);
      if (selectedNamesWithTypes.length > 0) {
      }
    } catch (error) {
